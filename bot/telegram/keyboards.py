...
def force_sub_keyboard(join_targets: list[tuple[str, str]]) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    for title, link in join_targets:
        rows.append([InlineKeyboardButton(f"📢 Join Channel • {title}", url=link)])
