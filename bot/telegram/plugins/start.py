...
async def ensure_user_access(client: Client, message: Message) -> bool:
...
async def search_command_handler(client: Client, message: Message) -> None:
    try:
        if not await ensure_user_access(client, message):
            return
...
async def users_handler(client: Client, message: Message) -> None:
    try:
        if not await ensure_user_access(client, message):
            return
