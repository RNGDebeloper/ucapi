...
class BotDatabase:
    ...
    async def _ensure_content_indexes(self) -> None:
        required_fields = {"title", "tags", "search_blob"}
        ...
        for index_name, meta in info.items():
            keys = meta.get("key", [])
            # Mongo text indexes contain ('_fts', 'text') in the key metadata.
            if any(k == "_fts" and v == "text" for k, v in keys):
                text_indexes.append((index_name, meta))

        has_correct_text_index = False
        for index_name, meta in text_indexes:
            LOGGER.info("Existing index found: %s", index_name)
            weights = {k: int(v) for k, v in (meta.get("weights") or {}).items()}
            existing_fields = set(weights.keys())

            if existing_fields == required_fields and all(weights.get(k, 1) == 1 for k in required_fields):
                has_correct_text_index = True
                LOGGER.info("Index already correct, skipping")
                continue

            try:
                LOGGER.info("Dropping conflicting index: %s", index_name)
                await self.content.drop_index(index_name)
            except PyMongoError as err:
                LOGGER.error("Failed to drop conflicting index %s: %s", index_name, err, exc_info=True)

        if not has_correct_text_index:
            try:
                LOGGER.info("Creating new index: %s", self.CONTENT_TEXT_INDEX_NAME)
        ...
        try:
            ...
            LOGGER.error("Failed to ensure non-text content indexes: %s", err, exc_info=True)
